...
ftt_orig = FTTransformer(
    n_cont_features=n_cont_features,
    cat_cardinalities=cat_cardinalities,
    d_out=d_out,
    **FTTransformer.get_default_kwargs(),
).to(device)
ftt_orig_optim = ftt.make_default_optimizer()

ftt_b = FTTransformer(
    n_cont_features=0,
    cat_cardinalities=cat_cardinalities_b,
    d_out=d_out,
    **FTTransformer.get_default_kwargs(),
).to(device)
ftt_b_optim = ftt.make_default_optimizer()

models = [ftt_orig, ftt_b]
optimizers = [ftt_orig_optim, ftt_b_optim]
...
print(f'FTTransformer ORIG val score before training: {evaluate(ftt, "val"):.4f}')
...
model_names = ['ftt_orig', f'ftt_{word_access}']
...
for epoch in range(n_epochs):
    ...
    for i, (model, e_s) in enumerate(zip(models, early_stoppings)):
        ...
        if val_score > best[i]["val"]:
            ...
            torch.save(model.state_dict(), f'models/{model_names[i]}_{word_access}_best_val')
        print()
...
for i, model in enumerate(models):
    ...
    model_class.load_state_dict(torch.load(f'models/{model_names[i]}_{word_access}_best_val'))
    ...
    with open(f'predictions/{model_names[i]}_{word_access}_data_predictions.json', 'w') as fp:
        json.dump(model_data_preds, fp)f