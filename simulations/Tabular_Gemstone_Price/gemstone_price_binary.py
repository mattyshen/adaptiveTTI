...
X_b = {}
bt = BinaryTransformer(depth = 1, bit = False)
...
for epoch in range(n_epochs):
    ...
    for i, (model, e_s) in enumerate(zip(models, early_stoppings)):
        ...
        if val_score > best[i]["val"]:
            ...
            torch.save(model.state_dict(), f'models/{model_names[i]}_binary_best_val')
        print()
...
for i, model in enumerate(models):
    ...
    model_class.load_state_dict(torch.load(f'models/{model_names[i]}_binary_best_val'))
    ...
    with open(f'predictions/{model_names[i]}_binary_data_predictions.json', 'w') as fp:
