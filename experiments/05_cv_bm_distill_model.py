...
import interpretDistill.featurizer
...
if __name__ == "__main__":
    ...
    for train_index, test_index in kf.split(X_train):
        X_out, y_out = X_train[test_index, :], y_train[test_index]
        X_in, y_in = X_train[train_index, :], y_train[train_index]
        for i, bm_param in enumerate(bm_params):
            ...
            distiller_k = interpretDistill.model.get_model(args.task_type, args.distiller_name, args)
            
            feature_names = list(X_in_k.columns)
            
            _, model_k = fit_model(model_k, X_in_k, y_in_k, feature_names, no_interaction, 0)

            y_train_teacher = pd.Series(model_k.predict(X_in_k), name = y_in_k.name)
            
            _, distiller_k = fit_model(distiller_k, X_in_k, y_train_teacher, feature_names, no_interaction, 0)

            scores_[i].append(r2_score(y_out, distiller_k.predict(X_out_k)))
    ...
    r, model = fit_model(model, X_train, y_train, feature_names, no_interaction, rrrr)

    y_train_teacher = pd.Series(model.predict(X_train), name = y_train.name)
