...
def add_main_args(parser):
    ...
    parser.add_argument(
        "--distiller_name",
        type=str,
        choices=["ft_distill", "figs"],
        default="ft_distill",
        help="name of distiller model",
    )
    parser.add_argument(
        "--featurizer_name", type=str, default="gmm_binary_mapper", help="type of featurizer to discretize dataset"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_features", type=float, default=1, help="max features of RF, RF+, XGB model"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    X_train_new = featurizer.fit_transform(X_train)

    X_val_new = featurizer.transform(X_val)
    ...
    # fit
    r, model = fit_model(model, X_train_new, y_train, feature_names, no_interaction, r)
    y_train_teacher = model.predict(X_train_new)
    y_train_teacher = pd.Series(y_train_teacher, name = y_train.name)
    r, distiller = fit_model(distiller, X_train_new, y_train_teacher, feature_names, no_interaction, r)
    
    r = evaluate_model(model, 'teacher', 'true', args.task_type, X_train_new, X_val_new, y_train, y_val, r)
    r = evaluate_model(distiller, 'distiller', 'true', args.task_type, X_train_new, X_val_new, y_train, y_val, r)
    r = evaluate_model(distiller, 'distiller', 'teacher', args.task_type, X_train_new, X_val_new, model.predict(X_train_new), model.predict(X_val_new), r)
